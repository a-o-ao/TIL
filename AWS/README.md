...
| [Storage Gateway](./Storage%20Gateway.md) | オンプレミスのストレージをAWSクラウドに接続するためのハイブリッドクラウドストレージサービス。 |
| [EBS](./EBS.md) | ブロックストレージサービス。 |
...
| [WAF](./WAF.md) | Webアプリケーションファイアウォール。 |
| Secrets Manager | シークレット管理とローテーションサービス。 |
| [GuardDuty](./GuardDuty.md) | 悪意のあるアクティビティや不正行為を検出するための脅威検出サービス。
